...
function changeMenu(id) {
  ...
  $(".page .content").fadeOut(500, function () {
    $(this)
      .empty()
      .load("pages/" + lang + "/" + id.substr(1).toLowerCase(), function () {
        $(".page .loading").hide();
...
function registerSideMenu(contentDiv) {
  ...
  $menu.find("button[href]").on("click", function (e) {
    ...
    $(contentDiv)
      ...
      .animate(
        {
          scrollTop: $(contentDiv).scrollTop() + $target.position().top - 26.72,
        },
        400,
        "swing",
        function () {
  ...
  function onScroll() {
    ...
    $menu.find("button[href]").each(function () {
      var yPos = $(contentDiv).find($(this).attr("href")).find("h2").position()
        .top;
...
function registerGalery($galeries) {
  $galeries.each(function (i) {
    ...
    setTimeout(function () {
      cssSlidy({
        slidyContainerSelector: "#" + galery.attr("id"),
        slidySelector: "#" + figure.attr("id"),
        captionSource: "alt",
      });
    }, 100 * i);
  });
